...
Docname: draft-xkumakichi-xaip-receipts-01
...
A distinguishing property of the format is mutual attestation: the Caller that delegated a tool call can co-sign the same canonical record as the Agent that executed it, so neither party can unilaterally fabricate a receipt about a delegated call. Co-signature is recommended rather than required; a receipt may carry the executor signature alone, and consumers may weight the two cases differently.
...
Other efforts also record evidence about AI agent actions. They differ from this document mainly in the unit of evidence and in which party attests. This appendix is informative and describes that adjacent work for context; this document does not depend on any of these formats.
